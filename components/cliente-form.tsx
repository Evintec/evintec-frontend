...
import MaskedInput from "./InputMaskControl"
import axios from 'axios';


// Definição do esquema completo do formulário
...
// Campos por etapa para validação e renderização
const stepFields = {
  0: [
    "codigo",
    "data_cadastro",
  ...
  1: ["logradouro", "numero", "complemento", "bairro", "cidade", "uf", "cep", "codigo_ibge", "pais"],
  2: ["contato", "telefone_principal", "celular", "telefone_contato", "email", "homepage"],
  3: ["limite", "creditobloqueado", "observacao"],
}
...
export default function ClienteForm() {
  ...
  const validateCurrentStep = async () => {
    if (currentStep >= 4) return true // Etapa de revisão não precisa de validação

    // Obter os campos da etapa atual
  ...
  const goToStep = (step: number) => {
    ...
    if (step > 0 && stepCompleted[step - 1]) {
      setCurrentStep(step)
    }
  }
  ...
  const sendData = async (data) => {
    try {
      const response = await axios.post('http://localhost:9000/cliente', data, {
        headers: {
  ...
  // Componente para a etapa 1: Dados Gerais
  const DadosGeraisStep = () => (
    <div className="space-y-4  slide-in-from-right-5 duration-300">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="CPF ou CNPJ" {...field} inputMode="numeric"/>
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
  ...
  // Componente para a etapa 3: Contato
  const ContatoStep = () => (
    <div className="space-y-4  slide-in-from-right-5 duration-300">
      <FormField
        control={form.control}
  ...
  // Componente para a etapa 4: Financeiro
  const FinanceiroStep = () => (
    <div className="space-y-4  slide-in-from-right-5 duration-300">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FormField
          control={form.control}
  ...
  // Componente para a etapa 5: Revisão
  const RevisaoStep = () => {
    const values = form.getValues()
    return (
      <div className="space-y-6  slide-in-from-right-5 duration-300">
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 0:
        return <DadosGeraisStep />
      case 1:
        return <EnderecoStep />
      case 2:
        return <ContatoStep />
      case 3:
        return <FinanceiroStep />
      case 4:
        return <RevisaoStep />
      default:
        return null
